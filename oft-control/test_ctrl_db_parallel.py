...
def process(
    input_image, prompt, hint_image, a_prompt, n_prompt, num_samples, image_resolution, ddim_steps,
    ...
    with torch.no_grad():
        ...
        cond = {
            "c_concat": [control], "c_crossattn":
            [model.get_learned_conditioning([prompt + ', ' + a_prompt] * num_samples)]
        }
        un_cond = {
            "c_concat": None if guess_mode else [control], "c_crossattn":
...
if __name__ == '__main__':
    ...
    experiment = './oft-control/log/image_log_oft_{}_{}_eps_{}_pe_diff_mlp_r_{}'.format(
        data_name, control, args.eps, args.r
    )
    db_path = "./oft-db/log_cot/yuliang-man-prior/"
    ...
    model.load_state_dict(load_state_dicts(resume_path, location='cuda'))
    model = model.cuda()
