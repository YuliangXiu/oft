...
class NoiseScheduleVP:
    ...
    def inverse_lambda(self, lamb):
        ...
        if self.schedule == 'linear':
            tmp = 2. * (self.beta_1 -
                        self.beta_0) * torch.logaddexp(-2. * lamb,
...
class DPM_Solver:
    ...
    def data_prediction_fn(self, x, t):
        ...
        dims = x.dim()
        alpha_t, sigma_t = self.noise_schedule.marginal_alpha(t
    ...
    def get_time_steps(self, skip_type, t_T, t_0, N, device):
        ...
        if skip_type == 'logSNR':
            ...
            logSNR_steps = torch.linspace(lambda_T.cpu().item(),
                                          lambda_0.cpu().item(), N + 1).to(device)
    ...
    def get_orders_and_timesteps_for_singlestep_solver(
        ...
        if skip_type == 'logSNR':
            ...
            timesteps_outer = self.get_time_steps(skip_type, t_T, t_0, steps, device)[torch.cumsum(
                torch.tensor([
                    0,
                ] + orders)
            ).to(device)]
        ...
        if self.predict_x0:
            ...
            if solver_type == 'dpm_solver':
                ...
                x_t = (
                    expand_dims(torch.exp(log_alpha_t - log_alpha_s), dims) * x -
                    expand_dims(sigma_t * phi_1, dims) * model_s -
                    (1. / r1) * expand_dims(sigma_t * ((torch.exp(h) - 1.) / h - 1.), dims) *
                    (model_s1 - model_s)
                )
        if return_intermediate:
    ...
    def singlestep_dpm_solver_third_update(
        ...
        alpha_s1, alpha_s2, alpha_t = torch.exp(log_alpha_s1), torch.exp(log_alpha_s2
    ...
    def multistep_dpm_solver_second_update(
        ...
        log_alpha_prev_0, log_alpha_t = ns.marginal_log_mean_coeff(t_prev_0
                                                                  ), ns.marginal_log_mean_coeff(t)
    ...
    def multistep_dpm_solver_third_update(
        ...
        lambda_prev_2, lambda_prev_1, lambda_prev_0, lambda_t = ns.marginal_lambda(
            t_prev_2
        ), ns.marginal_lambda(t_prev_1), ns.marginal_lambda(t_prev_0), ns.marginal_lambda(t)
        log_alpha_prev_0, log_alpha_t = ns.marginal_log_mean_coeff(t_prev_0
                                                                  ), ns.marginal_log_mean_coeff(t)
        sigma_prev_0, sigma_t = ns.marginal_std(t_prev_0), ns.marginal_std(t)
        alpha_t = torch.exp(log_alpha_t)
    ...
    def dpm_solver_adaptive(
        ...
        while torch.abs((s - t_0)).mean() > t_err:
            ...
            if torch.all(E <= 1.):
                ...
                lambda_s = ns.marginal_lambda(s)
            h = torch.min(
                theta * h * torch.float_power(E, -1. / order).float(), lambda_0 - lambda_s
            )
            nfe += order
...
def interpolate_fn(x, xp, yp):
    ...
    end_y = torch.gather(y_positions_expanded, dim=2,
