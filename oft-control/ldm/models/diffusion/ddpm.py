...
class DDPM(pl.LightningModule):
    ...
    def p_sample_loop(self, shape, return_intermediates=False):
        ...
        intermediates = [img]
        for i in tqdm(
            reversed(range(0, self.num_timesteps)), desc='Sampling t', total=self.num_timesteps
        ):
            img = self.p_sample(
                img,
...
class LatentDiffusion(DDPM):
    ...
    def get_fold_unfold(
        ...
        if uf == 1 and df == 1:
            ...
            weighting = self.get_weighting(kernel_size[0], kernel_size[1], Ly, Lx,
                                           x.device).to(x.dtype)
    ...
    def p_sample(
        ...
        if return_codebook_ids:
            return model_mean + nonzero_mask * (0.5 *
                                                model_log_variance).exp() * noise, logits.argmax(
                                                    dim=1
                                                )
        if return_x0:
    ...
    def progressive_denoising(
        ...
        if cond is not None:
            if isinstance(cond, dict):
                cond = {
                    key:
                    cond[key][:batch_size] if not isinstance(cond[key], list) else
                    list(map(lambda x: x[:batch_size], cond[key]))
                    for key in cond
                }
            else:
                cond = [c[:batch_size]
                        for c in cond] if isinstance(cond, list) else cond[:batch_size]
    ...
    def configure_optimizers(self):
        ...
        if self.use_scheduler:
            ...
            scheduler = [{
                'scheduler': LambdaLR(opt, lr_lambda=scheduler.schedule), 'interval': 'step',
                'frequency': 1
            }]
...
class LatentUpscaleDiffusion(LatentDiffusion):
    ...
    def log_images(
        ...
        **kwargs
        ...
        log["reconstruction"] = xrec
        log["x_lr"] = x_low
        log[f"x_lr_rec_@noise_levels{'-'.join(map(lambda x: str(x), list(noise_level.cpu().numpy())))}"
           ] = x_low_rec
        if self.model.conditioning_key is not None:
            if hasattr(self.cond_stage_model, "decode"):
                xc = self.cond_stage_model.decode(c)
...
class LatentInpaintDiffusion(LatentFinetuneDiffusion):
    ...
    def log_images(self, *args, **kwargs):
        ...
        log["masked_image"] = rearrange(args[0]["masked_image"], 'b h w c -> b c h w').to(
...
class LatentDepth2ImageDiffusion(LatentFinetuneDiffusion):
    ...
    def get_input(self, batch, k, cond_key=None, bs=None, return_first_stage_outputs=False):
        ...
        for ck in self.concat_keys:
            ...
            depth_min, depth_max = torch.amin(cc, dim=[
                1, 2, 3
            ], keepdim=True), torch.amax(cc, dim=[1, 2, 3], keepdim=True)
