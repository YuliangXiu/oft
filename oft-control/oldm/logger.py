...
class ImageLogger(Callback):
    def __init__(
        ...
        self.log_path = os.path.join(save_dir, "log", "image_log_{}".format(self.experiment))
    ...
    def init_loss_logger(self):
        ...
        logging.basicConfig(
            filename=os.path.join(self.log_path, "training_loss.log"),
            level=logging.INFO,
    ...
    def log_local(self, save_dir, split, images, global_step, current_epoch, batch_idx):
        ...
        for k in images:
            ...
            print(f"root: {root}", "save_dir: ", save_dir, "path: ", path)
            Image.fromarray(grid).save(path)

    @rank_zero_only
    def log_local_val(self, save_dir, split, images, global_step, current_epoch, batch_idx):
        root = os.path.join(
            save_dir, "log", "image_log_{}".format(self.experiment), split, str(current_epoch)
        )
        for k in images:
            for idx, image in enumerate(images[k]):
                ...
                filename = "{}_gs-{:06}_e-{:06}_b-{:06}_i-{:06}.png".format(
                    k, global_step, current_epoch, batch_idx, idx
                )
                path = os.path.join(root, filename)
                os.makedirs(os.path.split(path)[0], exist_ok=True)
                print(f"root: {root}", "save_dir: ", save_dir, "path: ", path)
    ...
    def log_img_val(self, pl_module, batch, batch_idx, split="val"):
        check_idx = batch_idx    # if self.log_on_batch_idx else pl_module.global_step
        if (
            hasattr(pl_module, "log_images") and callable(pl_module.log_images) and
            self.max_images > 0
        ):
            logger = type(pl_module.logger)
            ...
            with torch.no_grad():
                images = pl_module.log_images(
                    batch, split=split, num_samples=self.num_samples, **self.log_images_kwargs
                )
