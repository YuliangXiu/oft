    ...
    for _module, name, _child_module in _find_modules(
        ...
        if isinstance(_child_module, LoraInjectedLinear):
            ...
            _child_module.conv.weight = nn.Parameter(
                ...
                type(_child_module.conv.weight.dtype).to(_child_module.conv.weight.device)
            )
...
def monkeypatch_or_replace_lora(
    ...
    for _module, name, _child_module in _find_modules(
        ...
        _source = (
            _child_module.linear if isinstance(_child_module, LoraInjectedLinear) else _child_module
        )
...
def monkeypatch_or_replace_lora_extended(
    ...
    for _module, name, _child_module in _find_modules(
        ...
        if (_child_module.__class__
            == nn.Linear) or (_child_module.__class__ == LoraInjectedLinear):
            if len(loras[0].shape) != 2:
                continue

            _source = (
                _child_module.linear
                if isinstance(_child_module, LoraInjectedLinear) else _child_module
            )
        ...
        elif (_child_module.__class__
              == nn.Conv2d) or (_child_module.__class__ == LoraInjectedConv2d):
            if len(loras[0].shape) != 4:
                continue
            _source = (
                _child_module.conv
                if isinstance(_child_module, LoraInjectedConv2d) else _child_module
            )
...
def apply_learned_embed_in_clip(
    ...
    if isinstance(token, str):
        ...
        assert len(
            learned_embeds.keys()
        ) == len(token), "The number of tokens and the number of embeds should be the same"
        trained_tokens = token
    else:
