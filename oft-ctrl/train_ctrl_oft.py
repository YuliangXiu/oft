...
def log_validation(
    val_dataset, vae, text_encoder, tokenizer, unet, controlnet, args, accelerator, weight_dtype,
    step
):
    ...
    for idx in range(args.num_validation_images):
        ...
        validation_prompt = data['caption']
        validation_image = data['conditioning_pixel_value']
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--rank",
        type=int,
        help=("The factor to divide the orthogonal matrix to smaller blocks."),
    )

    parser.add_argument(
        "--coft",
        action='store_true',
        help=("The constrainted variant of OFT."),
    )
...
def main(args):
    ...
    accelerator.register_for_checkpointing(controlnet)
    ...
    optimizer = optimizer_class(
        [{'params': oft_layers.parameters(), 'params': controlnet.parameters()}],
    ...
    # Load the dataset
    train_dataset = ControlNetDataset(split="train", resolution=args.resolution)
    ...
    controlnet, oft_layers, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        controlnet, oft_layers, optimizer, train_dataloader, lr_scheduler
    )
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("controlnet-oft", config=vars(args), init_kwargs=wandb_init)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet), accelerator.accumulate(controlnet):
                ...
                if accelerator.sync_gradients:
                    ...
                    accelerator.clip_grad_norm_(oft_layers.parameters(), args.max_grad_norm)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.validation_steps == 0:
                        ...
                        log_validation(
                            val_dataset, vae, text_encoder, tokenizer, unet, controlnet, args,
