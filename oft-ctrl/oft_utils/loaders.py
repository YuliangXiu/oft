...
class UNet2DConditionLoadersMixin:
    ...
    def load_attn_procs(
        ...
        if not isinstance(pretrained_model_name_or_path_or_dict, dict):
            ...
            if (use_safetensors and weight_name
                is None) or (weight_name is not None and weight_name.endswith(".safetensors")):
                try:
        ...
        if is_lora:
            ...
            for key, value_dict in lora_grouped_dict.items():
                ...
                if isinstance(
                    ...
                    if isinstance(
                        attn_processor, (XFormersAttnProcessor, LoRAXFormersAttnProcessor)
                    ):
        ...
        **kwargs,
        ...
        if is_custom_diffusion:
            model_to_save = AttnProcsLayers({
                y: x
                for (y, x) in self.attn_processors.items() if
                isinstance(x, (CustomDiffusionAttnProcessor, CustomDiffusionXFormersAttnProcessor))
            })
...
class TextualInversionLoaderMixin:
    ...
    def load_textual_inversion(
        self,
        pretrained_model_name_or_path: Union[str, List[str], Dict[str, torch.Tensor],
...
class LoraLoaderMixin:
    ...
    def load_lora_weights(
        ...
        if all(
            key.startswith(self.unet_name) or key.startswith(self.text_encoder_name) for key in keys
        ):
    ...
    def _load_text_encoder_attn_procs(
        ...
        # set correct dtype & device
        attn_processors = {
            k: v.to(device=self.device, dtype=self.text_encoder.dtype)
            for k, v in attn_processors.items()
        }
    ...
    def save_lora_weights(
        ...
        if unet_lora_layers is not None:
            weights = (
                ...
                if isinstance(unet_lora_layers, torch.nn.Module) else unet_lora_layers
            )
        ...
        if text_encoder_lora_layers is not None:
            weights = (
                text_encoder_lora_layers.state_dict() if
                isinstance(text_encoder_lora_layers, torch.nn.Module) else text_encoder_lora_layers
            )
            ...
            if "lora_down" in key:
                ...
                if lora_name.startswith("lora_unet_"):
                    ...
                    if "self_attn" in diffusers_name:
                        ...
                        te_state_dict[diffusers_name.replace(".down.",
